...
import { PaginationControls } from "../commons/pagination-controls";
import {
  FilterState,
  Transaction,
  TransactionStatus,
...
interface ProfileContentTransactionProps {
...
export default function ProfileContentTransaction({
  formatPrice,
  formatDate,
}: ProfileContentTransactionProps) {
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="w-full flex flex-wrap items-center justify-center gap-4">
          {statusFilterBarOptions.map((option) => {
            const isActive =
              (option.key === "all" && filters.status === "all") ||
              (option.key !== "all" && String(option.key) === filters.status);
