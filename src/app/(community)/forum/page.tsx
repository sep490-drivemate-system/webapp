...
import { ReactionType as ForumReactionType, PostStatus } from "@/types/forum/post.enum";
...
export default function ForumPage() {
    ...
    const filteredPosts = useMemo(() => {
        ...
        return posts.filter((post) => {
            ...
            // Other roles (Admin, Inspector): see Approved posts and their own Pending posts
            return isApproved || (isPending && isOwnPost);
        });
    }, [posts, currentUser]);
                        ...
                        {isLoadingPosts ? (
                            ...
                            finalFilteredPosts.map((post) => {
                                ...
                                const sortedImages = post.images
                                    ? [...post.images].sort((a, b) => (a.order || 0) - (b.order || 0))
                                    : [];
                                ...
                                type MediaItem = { type: 'image'; url: string; order: number } | { type: 'video'; url: string; order: number };
                                const mediaItems: MediaItem[] = [
                                    ...sortedImages.map(img => ({ type: 'image' as const, url: img.url, order: img.order || 0 })),
                                    ...sortedVideos.map(vid => ({ type: 'video' as const, url: vid.url, order: vid.order }))
                                ].sort((a, b) => a.order - b.order);

                                const hasMedia = mediaItems.length > 0;

                                return (
                                    <Card key={post.postId} className="overflow-hidden hover:shadow-xl transition-all duration-300 border-l-4 border-l-transparent ">
                                        <CardContent className="pt-6">
                                                ...
                                                <div className="flex items-start gap-3 flex-1">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <span className="font-bold text-base">{post.authorName}</span>
                                                            {isOwnPost && isPending && (
                                            ...
                                            <div className="flex items-center justify-between text-sm text-muted-foreground mb-4 pb-4 border-b border-gray-200">
                                                <div className="flex items-center gap-6">
                                                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-100 ">
                                                        ...
                                                        <span className="font-medium">{post.likeCount || post.reactions?.length || 0}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-100 ">
                                            ...
                                            <div className="flex items-center gap-2 border-t border-gray-100 pt-3">
                                                <Button
                                                    variant="ghost"
                                                    className="flex-1 "
                                                    onClick={() => handleReaction(post.postId, ForumReactionType.Like)}
                                                ...
                                                <div className="mt-4 space-y-4">
                                                        ...
                                                        <div className="flex items-start gap-2">
                                                            ...
                                                            <div className="flex-1 flex gap-2">
                                                                <Textarea
                                                                    ...
                                                                    onChange={(e) =>
                                                                        updateCommentInput(post.postId, e.target.value)
                                                                    }
                                                                    rows={2}
                                                                ...
                                                                <Button
                                                                    ...
                                                                    disabled={isCommenting[post.postId] || !commentInputs[post.postId]?.trim()}
                                                                >
                                                    ...
                                                    {isPending && !isOwnPost && (
                                                        <div className="text-center py-4 text-sm text-muted-foreground">
                                                            Bài viết đang được kiểm duyệt. Bạn không thể bình luận.
                                                        </div>
                                                    )}
                                                                    ...
                                                                    <div key={comment.commentId} className="flex items-start gap-3">
                                                                        ...
                                                                        <div className="flex-1">
                                                                                ...
                                                                                <div className="mt-2 flex items-start gap-2">
                                                                                    ...
                                                                                    <div className="flex-1 flex gap-2">
                                                                                        <Textarea
                                                                                            ...
                                                                                            value={replyInputs[`${post.postId}-${comment.commentId}`] || ""}
                                                                                            onChange={(e) =>
                                                                                                updateReplyInput(post.postId, comment.commentId, e.target.value)
                                                                                            }
                                                                                            rows={1}
                                                                                            className="resize-none text-sm"
                                                                                            disabled={isCommenting[`${post.postId}-${comment.commentId}`]}
                                                                                        />
                                                                                        <Button
                                                                                            size="icon"
                                                                                            variant="ghost"
                                                                                            onClick={() => handleReply(post.postId, comment.commentId)}
                                                                                            disabled={isCommenting[`${post.postId}-${comment.commentId}`] || !replyInputs[`${post.postId}-${comment.commentId}`]?.trim()}
                                                                                        >
                                                                                            {isCommenting[`${post.postId}-${comment.commentId}`] ? (
                                                                            ...
                                                                            {post.comments
                                                                                ...
                                                                                .map((reply) => (
                                                                                    <div key={reply.commentId} className="flex items-start gap-2 mt-2 ml-4">
                                                                                        <Avatar className="h-6 w-6">
                                                                                            {reply.authorAvatar ? (
